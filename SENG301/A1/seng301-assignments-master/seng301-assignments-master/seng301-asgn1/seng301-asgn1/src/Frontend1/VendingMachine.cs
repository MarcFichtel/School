...
namespace seng301_asgn1.src.Frontend1
{
    class VendingMachine
    {
        ...
        {
            loadedCoins.Add(coinKinds[coinKindIndex], coins);
        }
        ...
        public void PressButton (int button)
        {
            ...
            if (insertedCoinsValue >= price)
            {
                ...
                if (insertedCoinsValue > price)
                {
                    ...
                    {
                        changeAvailable += coinKinds[i] * loadedCoins[coinKinds[i]].Count;
                    }
                    ...
                    if (changeAvailable >= changeRequired)                                      // Check if enough change is present
                    {
                        while (changeRequired > 0)
                        {
                            ...
                            for (int h = 0; h < loadedCoins.Count; h++)                             // Cycle forward through coin kinds
                            {
                                ...
                                {
                                    for (int j = 0; j < loadedCoins[coinKinds[i]].Count; j++)       // Cycle through coins
                                    {
                                        // Add coin kind value to changeActual if the sum doesn't exceep changeRequired
                                        if (changeActual + coinKinds[i] <= changeRequired)
                                        {
                                            changeActual += coinKinds[i];
                                            changeCoins.Add(loadedCoins[coinKinds[i]][j]);          // Track this specific coin
                                        }
                                        ...
                                        if (changeActual == changeRequired)
                                        {
                                            ...
                                            foreach (Coin c in changeCoins)
                                            {
                                                ...
                                                for (int m = 0; m < loadedCoins.Count; m++)
                                                {
                                                    if (loadedCoins[coinKinds[m]].Contains(c))
                                                    {
                                                        loadedCoins[coinKinds[m]].Remove(c);        // Remove coin from loaded coins
                                                    }
                                                }
                                            }
                            ...
                            if (changeActual != changeRequired)
                            {
                                ...
                                for (int h = loadedCoins.Count - 1; h >= 0; h--)                        // Cycle backwards through coin kinds
                                {
                                    ...
                                    {
                                        for (int j = 0; i < loadedCoins[coinKinds[i]].Count; j++)       // Cycle through coins
                                        {
                            ...
                            // If a successful match was found, break out of while loop
                            if (changeActual == changeRequired)
                                break;

                            // Else decrement change required to look for next biggest possible combination
                            changeRequired--;
                        }
                    }
                }
