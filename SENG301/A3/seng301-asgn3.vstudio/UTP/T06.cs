    ...
    */
    ...
    public class T06 {
        ...
        public void Test06() {
            ...
            string[] expected1 = { null, null };             // Set up expected result
            ...
            // CHECK_DELIVERY(0)
            // TODO Check if its possible to assert two lists or arrays are the same
            string[] expected2 = { null, null };             // Set up expected result
            for (int i = 0; i < contents.Length; i++) {      // Iterate over contents
                Assert.AreEqual(contents[i], expected2[i]);  // Assert each content element is as expected
            }
            ...
            // CHECK_TEARDOWN(65; 0; "Coke", "water", "stuff")
            int expected3 = 65;                                                     // Variable holds expected result 2
            int expected4 = 0;                                                      // Variable holds expected result 3 
            List<string> expected5 = new List<string> { "Coke", "water", "stuff" }; // Variable holds expected result 4
            Assert.AreEqual(storedCoinsValue, expected3);                           // Assert that stored coins value is as expected
            Assert.AreEqual(storageBinValue, expected4);                            // Assert that storage bin value is as expected
            for (int i = 0; i < pops.Count; i++) {                                  // Iterate over pops
                Assert.AreEqual(pops[i], expected5[i]);                             // Assert each unloaded pop is as expected
            }
        }
    }
}
