...
namespace RuntimeGizmos
{
	...
	public class TransformGizmo : MonoBehaviour
	{
		...
		// Only get appropriate targets (i.e. selectable objects)
		void GetTarget() {
			...
			if (selectedAxis == Axis.None && Input.GetMouseButtonDown(0)) {
				...
				// If object is editable and no other object is selected and the distance to object is not too large
				if (Physics.Raycast(myCamera.ScreenPointToRay(Input.mousePosition), out hitInfo) && 
					hitInfo.transform.parent.parent.name == "Objects" &&
					target == null &&
					Vector3.Distance(transform.position, SelectionController.Instance.getSelected().transform.position) <= SelectionController.Instance.getHighlightingDistance()) {

					// Get raycast hit info
					target = hitInfo.transform;
				}

			// Set traget to null when done editing the object
			} else if (!GameController.Instance.isEditingObject()) {
				target = null;
			}
		}
